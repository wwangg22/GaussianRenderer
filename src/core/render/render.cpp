...
void transformAndTileGaussians(std::vector<Gaussian>& g, std::vector<lightWeightGaussian>& out, Camera& cam, 
    ...
    for (int idx = 0 ; idx < g.size(); idx++) {
        ...
        float X, Y, Z;
        // print old_xyz
        // std::cout << "Gaussian " << idx << " world coords: ";
        // std::cout << old_xyz[0] << ", " << old_xyz[1] << ", " << old_xyz[2] << "\n";
        ...
        // covar = R * S * R_T
        // update: covar is R * S * S * R_T
        MatMul_3D(R, scale_mat, tmp);
        MatMul_3D(tmp, scale_mat, R);
        MatMul_3D(R, R_T, covar);
        ...
        Sigma2D[0] = (tile_info.W * 0.5f) * (tile_info.W * 0.5f) * Sigma2D[0];
        Sigma2D[1] = (tile_info.W * 0.5f) * (tile_info.H * 0.5f) * Sigma2D[1];
        Sigma2D[2] = (tile_info.H * 0.5f) * (tile_info.W * 0.5f) * Sigma2D[2];
        Sigma2D[3] = (tile_info.H * 0.5f) * (tile_info.H * 0.5f) * Sigma2D[3];  
        // print sigma2D
        // std::cout << "Projected 2D covariance Sigma2D for gaussian " << idx << ": \n";
        // std::cout << Sigma2D[0] << " " << Sigma2D[1] << "\n";
        // std::cout << Sigma2D[2] << " " << Sigma2D[3] << "\n";
        // add blur filter (this was done in the og repo)
        ...
        int tiles_x = max_x - min_x + 1;
        int tiles_y = max_y - min_y + 1;
        int tiles_covered = tiles_x * tiles_y;
        int total_tiles = tile_info.num_tile_x * tile_info.num_tile_y;
        ...
        // lwg.radix_id = (static_cast<uint64_t>(tile_id) << 32) | static_cast<uint32_t> (gauss.z);
        // print Z
        // std::cout << "Gaussian " << idx << " Z: " <<-Z << "\n";
        lwg.radix_id = static_cast<uint64_t> (-Z* 1e6f); // using depth for sorting
        lwg.gaussian_id = static_cast<uint32_t> (idx); 
        out.push_back(lwg);
...
// Gaussian *preprocessGaussiansForSorting(Gaussian * d_gaussians, int numGaussians, 
//                                         Camera& cam, TilingInformation& tile_info, float k){
